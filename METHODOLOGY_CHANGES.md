...
## 44. Explicit no-hazard warm-up windows in `rp_files`
- **What**: Hazard schedules now accept `None` in place of a raster path, for example `10:1:80:FL:None`. These entries define explicit no-hazard intervals in the scenario schedule but do not load or sample any raster.
- **Why**: The repaired baseline still has a startup transient even though its long-run collapse problem has been fixed. Encoding a common no-hazard warm-up window directly in `rp_files` ensures that baseline, hazard, and adaptation scenarios all equilibrate under identical pre-forcing conditions before the climate signal turns on.
- **Where**: `hazard_utils.py`, shared hazard-spec parser; `run_simulation.py`; `check_consistency.py`; `model.py`, hazard initialization; `tests/test_stock_flow_closure.py`.
- **Manuscript impact**: Scenario design should state that hazard schedules may include an explicit no-hazard warm-up period before the forcing files become active, rather than implying that the hazard process must start at simulation step 1.

## 45. Adaptive-expectations resilience target replaces bandit learning
- **What**: Removed the contextual-bandit adaptation layer and replaced it with a simpler hazard-conditional resilience-target rule. Each firm now maintains EWMAs of expected direct loss, realized direct loss, nearby observed direct loss, and supplier disruption. Every `decision_interval` steps the firm computes a perceived hazard-risk index as a weighted average of expected direct loss, nearby observed loss, and supplier disruption, maps that into a firm-specific resilience target, and invests up to a capped increment toward the target.
- **Why**: The bandit design was too sample-inefficient for the sparse-hazard setting and required additional reward-design choices that obscured the paper's main question. The adaptive-expectations target rule is more transparent, easier to interpret economically, and better aligned with the idea that firms harden when they perceive persistent worsening risk rather than because they solve a generic reinforcement-learning problem.
- **Where**: `agents.py`, adaptation state and target-selection logic; `model.py`, adaptation reporters and update loop; parameter files now use `adaptation_sensitivity_min`, `adaptation_sensitivity_max`, and `max_adaptation_increment`; `tests/test_stock_flow_closure.py` replaces bandit tests with proportional-target tests.
- **Manuscript impact**: Rewrite the adaptation subsection to describe adaptive expectations, perceived hazard risk, firm-specific resilience sensitivity, and proportional target adjustment. Remove references to contextual-bandit exploration, reward windows, discrete maintain/small/large action values, and inherited bandit memory.

## 46. Sensitivity analysis now targets resilience sensitivity
- **What**: Reworked `sensitivity_analysis.py` so the robustness exercise varies the firm-level resilience-sensitivity range rather than the old UCB exploration coefficient. The matched-seed ensemble sweep now compares four sensitivity bands and reports both time-series summaries and final-decade ensemble statistics.
- **Why**: Once the bandit was removed, `ucb_c` was no longer meaningful. The key adaptation parameter is now how aggressively firms translate perceived hazard risk into a resilience target, so the sensitivity workflow should test that mechanism directly.
- **Where**: `sensitivity_analysis.py`, `tests/test_sensitivity_analysis.py`, `README.md`, `AGENTS.md`, and manuscript sensitivity text.
- **Manuscript impact**: Replace the old exploration-coefficient discussion with a targeted robustness check on resilience sensitivity. Any figure caption or parameter-choice discussion that still mentions `c` or UCB should be removed.

## 47. Annualized risk-to-target resilience mapping
- **What**: Adjusted the resilience-target rule so firms no longer map the quarterly perceived hazard signal directly into a resilience target. Instead, they annualize the perceived per-step risk using `steps_per_year` and then apply firm-specific resilience sensitivity to that annualized burden.
- **Why**: The earlier target rule was too conservative because it treated one quarter of expected direct loss as if it were the full object of resilience planning. In repeated-hazard settings, firms should respond to the recurring annual burden implied by quarterly signals. Annualizing the perceived risk produces materially stronger but still interpretable resilience build-up without reintroducing a complex learning algorithm.
- **Where**: `agents.py`, `_target_resilience_from_risk()` and `_select_adaptation_action()`; `tests/test_stock_flow_closure.py`; README/AGENTS adaptation descriptions.
- **Manuscript impact**: The adaptation subsection should state explicitly that firms annualize the perceived hazard-risk signal before translating it into a resilience target.

## 48. Continuity-capital adaptation targets hazard-induced operating shortfall
- **What**: Completed the shift from direct-loss hardening to continuity-capital adaptation. Firms now maintain a `continuity_capital` stock (with `resilience_capital` retained as a backward-compatible alias), form expectations over hazard-induced operating shortfall, observe nearby shortfalls, and use a simple target-adjustment rule to build continuity capacity. During production, continuity capital closes a fraction of the hazard-induced output gap up to the labor-feasible demand ceiling, and that gap coverage also reduces residual supplier disruption. Direct flood-loss attenuation and recovery support remain, but they are now part of one broader continuity mechanism rather than the sole adaptation channel.
- **Why**: The earlier direct-hardening design improved direct losses only modestly and often did little for the macro outcomes the paper actually cares about. The core research question is whether firms can sustain operations under worsening climate stress, including through indirect network cascades. Targeting hazard-induced operating shortfall is a more parsimonious and better-aligned mechanism than optimizing direct loss alone or building separate knobs for every bottleneck class.
- **Where**: `agents.py`, continuity state, production-step gap coverage, and shortfall-based target rule; `model.py`, new continuity reporters and local-shortfall observation helper; `tests/test_stock_flow_closure.py`, continuity-gap and local-shortfall regressions; `run_simulation.py` and `sensitivity_analysis.py`, standalone continuity metadata aliases.
- **Manuscript impact**: Rewrite the adaptation subsection around continuity capital and hazard-induced operating shortfall rather than resilience capital and direct-loss-focused targeting. Supporting diagnostics should now emphasize continuity capital, operating shortfall, and residual never-hit cascade burden alongside direct-loss outputs.

## 49. Move continuity support upstream to residual input shortages
- **What**: Refined the continuity mechanism so it no longer adds output after the bottlenecks are computed. Continuity capital now covers a fraction of residual climate-disrupted input shortages after normal procurement, and realized production is then computed from the standard Leontief minimum using those effective inputs. The code also now reports raw versus residual supplier disruption and continuity input coverage directly.
- **Why**: The first continuity implementation improved production and consumption, but it did so by closing output gaps too late in the period logic. That preserved quantities while leaving the underlying scarcity wedge largely intact, which showed up as excessive inflation and weak real-liquidity outcomes. Moving the mechanism upstream keeps the adaptation rule parsimonious while targeting the source-side scarcity pressure more directly.
- **Where**: `agents.py`, procurement/production logic and continuity accounting; `model.py`, new continuity-input and raw-scarcity reporters; `tests/test_stock_flow_closure.py`, updated continuity regression; README/AGENTS/manuscript adaptation descriptions.
- **Manuscript impact**: Update the adaptation subsection so continuity capital is described as covering residual disrupted inputs before production is finalized, rather than as an ex-post output-gap closure device.

## 50. Replace phantom-input continuity with backup-supplier search
- **What**: Removed the phantom-input mechanism that fabricated synthetic continuity coverage (`continuity_input_coverage = continuity_capital × remaining_inputs_needed`). Replaced it with actual backup-supplier search: firms with continuity capital search for non-primary suppliers in the same sectors as their connected firms, sorted by price, and buy real goods via `sell_goods_to_firm()`. Also made `get_adapted_loss_fraction()` a pass-through (no direct damage attenuation) and removed the recovery-rate boost from resilience capital.
- **Why**: The phantom-input mechanism created goods outside the circular flow, which inflated production, raised revenue per worker, raised wages, raised unit costs, and raised prices — producing an ~89% price premium for adapted firms. Backup-supplier search preserves macro closure: all transactions use real cash and real inventory, money is conserved, and no extra goods are created.
- **Where**: `agents.py` (backup search loop, pass-through `get_adapted_loss_fraction`, recovery rate, input consumption loop, `sell_goods_to_firm` key handling); `model.py` (`find_backup_suppliers()`, `max_backup_suppliers` config, backup-purchase data collectors); `run_simulation.py` and `sensitivity_analysis.py` (metadata); parameter files (`max_backup_suppliers` replaces `loss_reduction_max`); `tests/test_stock_flow_closure.py`.
- **Manuscript impact**: Update the adaptation mechanism description: continuity capital enables backup-supplier search through real market transactions, not phantom input creation. Describe the remaining ~25-30% price premium as a legitimate scarcity effect from redistributing scarce goods in a capacity-constrained economy.

## 51. Three selectable adaptation strategies
- **What**: Introduced a configurable `adaptation_strategy` parameter with three options:
  1. **Backup supplier search** (`backup_suppliers`): Firms with continuity capital search for alternative input suppliers when primary suppliers are disrupted. Maintains production through supply-chain diversification.
  2. **Capital hardening** (`capital_hardening`): Continuity capital directly attenuates physical damage from hazard events via `get_adapted_loss_fraction()`. Higher continuity → less capital/inventory/damage destruction.
  3. **Precautionary stockpiling** (`stockpiling`): Continuity capital increases the firm's target inventory buffer proportionally to perceived hazard risk. Firms build excess inventory during calm periods and draw down during disruptions.
  All three strategies share the same investment/decay/maintenance infrastructure and are completely dormant under baseline (no hazard) conditions. Strategy selection is a single config parameter.
- **Why**: The backup-supplier strategy maintains production but causes the highest inflation (scarcity pricing from inventory redistribution). Different strategies produce different macro tradeoffs: backup search prioritizes production continuity at the cost of inflation; capital hardening preserves physical capacity at the cost of lower throughput; stockpiling front-loads inventory at the cost of calm-period production. Comparing these strategies maps the adaptation-inflation tradeoff frontier — relevant to recent literature finding that climate risk shows up as higher inflation.
- **Where**: `agents.py` (`adaptation_strategy` attribute, strategy-branching in `get_adapted_loss_fraction`, backup search guard, `plan_operations` inventory target, `copy_adaptation_state_from`); `model.py` (strategy config and data collector); `run_simulation.py` (`--adaptation-strategy` CLI arg, strategy-aware scenario labels and metadata); `sensitivity_analysis.py` (metadata); parameter files; `tests/test_stock_flow_closure.py` (strategy-parameterized tests, dormancy tests for all strategies, money conservation tests).
- **Manuscript impact**: The scenario analysis expands from 3 to 5 scenarios (Baseline, Hazard-NA, Hazard + Backup Suppliers, Hazard + Capital Hardening, Hazard + Stockpiling). The results section should compare macro outcomes across strategies, emphasizing the production-vs-inflation tradeoff. The methodology should describe all three strategies as alternative channels through which continuity capital can operate. The externality structure matters: backup search has negative externalities (drains others' inventory), capital hardening is non-rival (protects own assets), stockpiling has positive externalities (relieves scarcity pressure during crises).
