...
- network-evolution PNGs and companion `*_network_evolution.json` snapshot files when network-evolution figures are rendered.

The saved network-evolution JSON records firm metadata, selected panel indices, active and inactive firm sets, and directed supplier edges for each stored snapshot. It can be replayed with `run_simulation.py --network-evolution-json ... --out ...` to regenerate the sector-level network figure without rerunning the simulation.
